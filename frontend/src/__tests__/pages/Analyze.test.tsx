...
describe("Analyze Page", () => {
  ...
  it("fetches and displays chart controls when a project is clicked", async () => {
    ...
    // Wait for checkboxes to appear, then assert
    expect(await screen.findByLabelText(/Gas/)).toBeChecked();
    expect(await screen.findByLabelText(/Diesel/)).toBeChecked();
    expect(await screen.findByLabelText(/Total CO₂/)).toBeChecked();
  });
});
